...
Sekarang sudah ada **sistem login** dengan 3 akun (1 admin, 2 staff) —
lihat bagian [Login & Akun](#login--akun) di bawah. Data presensi tiap
akun terpisah otomatis, jadi tidak lagi "data demo" yang sama untuk semua orang.

Struktur file sengaja dibuat **seringkas mungkin** (1 file CSS, 6 file
...
**Staff** (Asep, Rezky) hanya melihat & mencatat presensinya sendiri —
persis seperti sebelumnya, tapi sekarang datanya benar-benar terpisah
per akun (tersimpan dengan ID `{username}_{tanggal}`, jadi check-in
Asep tidak akan tertukar dengan Rezky).

**Admin** melihat data **gabungan semua staff** di halaman Riwayat
(tabel dapat tambahan kolom **Nama**) dan Dashboard (kartu & grafik
menjumlahkan semua staff, bukan cuma satu orang).

Halaman `index.html`, `absensi.html`, `riwayat.html`, `profil.html`
sekarang dilindungi **guard login**: kalau belum ada sesi aktif,
otomatis dilempar ke `login.html` sebelum konten sempat terlihat.
Tombol **Keluar** (ikon di sidebar, dekat nama akun) menghapus sesi
dan kembali ke halaman login.

> ⚠️ **Catatan keamanan**: ini autentikasi sisi-klien murni (daftar
> akun ada di `js/auth.js`, bisa dilihat lewat "View Source" browser).
> Cukup untuk demo/prototipe internal, tapi **belum aman untuk data
> sungguhan** — kalau serius dipakai, ganti dengan autentikasi
> sungguhan (misalnya Firebase Authentication) saat pindah dari
> localStorage ke Firebase.

## Struktur folder

```
absensi-dashboard/
├── login.html            # Halaman login (3 akun demo)
├── index.html            # Dashboard: statistik & grafik Chart.js (admin: semua staff)
├── absensi.html          # Absensi: kartu Check-in / Check-out
├── riwayat.html          # Riwayat: tabel presensi + filter (admin: semua staff + kolom Nama)
...
└── js/
    ...
    │                        # identitas akun aktif, dark mode, toast, spinner, sidebar mobile, jam
...
Tiap halaman terproteksi memuat **1 file CSS + 3 file JS**
(`auth.js` + `core.js` + satu file fitur halaman itu sendiri).
...
- **Multi-karyawan**: sudah aktif sejak sistem login ditambahkan — `CURRENT_EMPLOYEE` (bagian "app-config" di `core.js`) otomatis mengikuti akun yang sedang login (lihat [Login & Akun](#login--akun)). Menambah karyawan baru cukup menambah satu baris di `USERS` (`js/auth.js`), tidak perlu ubah struktur data.
